...
// GPIO
#define ST7789V_DC_CMD_MODE      0
#define ST7789V_DC_DATA_MODE     1
...
#define ST7789V_CHANNEL_CLK_SPEED   10000000
#define ST7789V_CHANNEL_QUEQUE_SIZE 5


uint16_t ST7789V::color565(uint8_t r, uint8_t g, uint8_t b)
{
    return ((r & 0xF8) << 8) | ((g & 0xFC) << 3) | ((b & 0xF8) >> 3);
}

ST7789V::ST7789V()
: DisplayGFX(ST7789V_TFTWIDTH, ST7789V_TFTHEIGHT)
{
}

#define ST7789V_SPI_TRANS_MAX 5
static spi_transaction_t _st7789vSpiTrans[ST7789V_SPI_TRANS_MAX];

void ST7789V::_initBus()
{
    ...
    _spiChannel.setParams(0, PIN_NUM_CS, ST7789V_CHANNEL_QUEQUE_SIZE, ST7789V_CHANNEL_CLK_SPEED);
    _spiChannel.bindTransactionCache(_st7789vSpiTrans, ST7789V_SPI_TRANS_MAX);
...
void ST7789V::turnOn(bool on)
{
    gpio_set_level((gpio_num_t)PIN_NUM_BCKL, on ? 1 : 0);
}

void ST7789V::_fireResetSignal()
{
...
{
    gpio_set_level((gpio_num_t)PIN_NUM_DC, ST7789V_DC_CMD_MODE);
    _spiChannel.tx(cmd);
    gpio_set_level((gpio_num_t)PIN_NUM_DC, ST7789V_DC_DATA_MODE);
}
...
void ST7789V::writeData(const uint8_t *data, uint16_t count)
{
    gpio_set_level((gpio_num_t)PIN_NUM_DC, ST7789V_DC_DATA_MODE);
...
// static const ST7789VInitCmd st7789vInitCmd[]={
//     {0x11, {0}, 0x80},
...
static const ST7789VInitCmd st7789vInitCmd[]={
    ...
    {0xC6, {0x0F}, 1},
    {0xD0, {0xA4, 0xA1}, 2},                   // pwer control: AVDD=6.8V, AVCL=-4.8V, VDS=2.3V
    {0xE0, {0xD0, 0x06, 0x0B, 0x0A, 0x09, 0x06, 0x2F, 0x44, 0x45, 0x18, 0x14, 0x14, 0x27, 0x2D}, 14},
    {0xE1, {0xD0, 0x06, 0x0B, 0x0A, 0x09, 0x05, 0x2E, 0x43, 0x45, 0x18, 0x14, 0x14, 0x27, 0x2D}, 14},
    {0x21, {0},    0},
    {0x35, {0x00}, 1},
    {0x44, {0x00, 0x00}, 2},
    {0xE7, {0x10}, 1},
...
void ST7789V::setRotation(uint8_t m)
{
...
{
    writeCommand(i ? ST7789V_INVON : ST7789V_INVOFF);
}

void ST7789V::scrollTo(uint16_t y)
{
    writeCommand(ST7789V_VSCRSADD);
    _spiChannel.tx16(y);
}
...
void ST7789V::setViewPort(uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
    ...
    _spiChannel.tx32(xa);
    writeCommand(ST7789V_PASET); // Row addr set
    _spiChannel.tx32(ya);
    writeCommand(ST7789V_RAMWR); // write to RAM
}

void ST7789V::writePixel(uint16_t color)
{
...
void ST7789V::writeColor(uint16_t color, uint32_t len)
{
...
uint8_t ST7789V::readcommand8(uint8_t c, uint8_t index)
{
...
// This code was ported/adapted from https://github.com/PaulStoffregen/ST7789V_t3
// by Marc MERLIN. See examples/pictureEmbed to use this.
void ST7789V::drawBitmap(int16_t x, int16_t y, int16_t w, int16_t h, const uint16_t *pcolors)
{
...
// //  for(i=2; i<n; i+=6) {
// //    i2 = i / 2;
// //    fillRect(cx-i2, cy-i2, i, i, ST7789V_GREEN);
// //  }
//     i2 = n / 2;

// //    for(int j=0; j<10; ++j) {
// //    fillRect(cx-i2, cy-i2, n, n, ST7789V_RED);
// //    drawFastHLine(15, 50, 30, ST7789V_CYAN);
// //    drawCircle(120, 160, 50, ST7789V_YELLOW);
